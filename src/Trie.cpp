...
Trie::Trie(std::string fileName) {
    ...
    readFile(fileName); // Read in all original Wordle words.txt and insert it into the trie starting at root
}
...
void Trie::playWordle() {
    ...
    std::cout << "Number of words.txt you have to work with: ";
    ...
    if (didWin != "yes" && didWin != "y") {
        ...
        std::cout << "Enter number of green letters: "; // Do green first because this eliminates most possible words.txt
        std::cin >> numGreen;
        if (numGreen != 0){
        ...
        std::cout << "Enter number of grey letters: "; // Do green second because this will remove more possible words.txt
        std::cin >> numGrey;
        if (numGrey != 0){
            removeGrey(numGrey);
        }
        ...
        if (numYellow != 0){                           // Doing grey and green first will reduce the amount of words.txt
        ...
        std::cout << "Here are your current valid words.txt: " << std::endl;
    ...
    while (i < numOfGuesses && (didWin != "y" && didWin != "yes")) {
        ...
        if (this->trieSize != 1) {
            std::cout << "You have cut down the amount of possible words.txt by " << percentChange << '%' << std::endl;
            std::cout << "Number of words.txt you have to work with: ";
            std::cout << this->trieSize << std::endl;
            std::cout << std::endl;
        }
...
void Trie::insert(std::string currWord) {
    ...
    for (int i = 0; i < currWord.size(); ++i) {
        letterLocation = currWord[i] - 'a'; // 'z' minus 'a' ==  Index 25, 'c' - 'a' == 2 Index 2 etc.
...
void Trie::printWords(Node *currNode, std::string currString){
    ...
    if (currNode->leafNode == true){             // If we got to the last node, that means we have reached the end
        ...
        this->trieSize++; // Update the size of the trie,so we can equate the difference of words.txt everytime
        return;
    }
...
void Trie::removeGreen(int numGreen) {
    ...
    // For each green letter and index, check the current valid words.txt and delete non-corresponding words.txt
...
void Trie::removeGreyChars(Node *currNode, Node *parentNode, int currI, char currChar, char greyChar) {
    if (currChar == greyChar){ // If the current char were on is grey, we can delete it because it is not needed
                              // in the possible list of words.txt
        delete parentNode->children[currI];
...
void Trie::removeYellow(int numYellow) {
    ...
    // For each yellow letter check the current trie and delete non-corresponding words.txt
...
//// it is yellow, that word can NOT be an answer. Example: Calfs, L is yellow at position 3. All words.txt
/// must include the letter L, but since L was not green, words.txt with L at position 3 are invalid.
...
void Trie::readFile(std::string fileName) {
    ...
    while (std::getline(file, str)) { // read in file line by line and append it to a vector
        insert(str); // Append to the trie word by word to save memory on using a vector of words.txt
        this->trieSize++;
